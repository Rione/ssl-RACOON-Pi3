...
## 会場に着いたら: コートの traction を測る

床の状態は**場所で変わる**。滑っている場所で取った追従の数字は信用できないし、
車輪と vision の食い違いの検査が発動して走れないこともある。
**使う場所ごとに 1 回ずつ測ってから始める。**

```bash
ROBOT=<ip> VENUE=<会場名> scripts/traction-survey.sh <場所の名前> [本数]
```
...
|---|---|
| 0.95 前後 | 正常 |
| 0.9 未満 | **滑っている。**ここで取った数字は使わない |
| 0.0 のまま | vision が別の物を見ている (traction の問題ではない) |

結果は `trajpoc-dataset/traction-survey.csv` に貯まる (git で追う)。
場所を変えて繰り返せば、コートのどこが滑るかの地図になる。

記録した走行から後で出すこともできる:

```bash
trajpoc-dataset/scripts/trajpoc-traction.py trajpoc-results/trajpoc-*.csv
```
...
- PoC の間は RAVEN との接続処理 (DISCOVER など) を起動しない。同じ DIP の ID の別機体と RAVEN の上で
  取り合いにならないように。万一 PC から指令が届いても反映しない。キックもしない (コンデンサの充電も止める)
