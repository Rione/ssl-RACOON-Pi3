...
- **書き直しの順序を守る。フィルタより先に検証を移す。**
  ...
  **正解表には「vision を基準にした向きのずれ」を必ず入れること。**
  2026-09-24、推定器に毎周期「4 輪とも 0」が渡っていた不具合を、上の指標が
  **1 つも捕まえなかった** (NEES 0.67 / NIS 0.52 / スムーザとの向き誤差 0.147° と、
  全部「健全」に見えていた)。疑似真値の RTS スムーザは**フィルタと同じモデルを共有する**ので、
  系統的な誤りには両方が同じだけ外れて見えない。**外から来る基準は vision だけ。**
  その場回転で推定の向きが vision から何度ずれるかを見ると、22° で一発で分かる。
...
- (解決) **推定器を制御に通すこと**。2026-09-24 に実施。
  その場回転で破綻 (向き 76°) したが、原因は推定器ではなく `feedEstimator` が
  車輪を読む前に呼ばれていたことだった。直して**向き 1.56°、`est_omega/gyro` 1.00**。
  直線・円・8 の字は vision の生値と同等で、**指令はむしろ滑らかになる** (円で 1.24 → 0.96)。
